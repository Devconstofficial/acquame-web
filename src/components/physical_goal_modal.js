...
import PhysicalBanner from "../asset/images/physical_banner.png";
import { media } from "../responsive";
import "@fontsource/roboto";
import ModalComponent from "../components/add_physical_goal_modal";
import ModalComponent1 from "../components/update_physical_goal_modal";
...
const StyledSlider = styled.input.attrs({ type: "range" })`
  ...
  background: ${({ value, max }) =>
    `linear-gradient(to right, ${colors.kPrimaryColor} ${
      (value / max) * 100
    }%, ${colors.kDarkPinkColor} ${(value / max) * 100}%)`};
  border-radius: 1rem;
  outline: none;
  cursor: pointer;
...
function PhysicalGoalModal({ isOpen, onClose }) {
  ...
  return (
    <ModalOverlay>
      <ModalContainer>
        ...
        <Banner>
          <BannerText>
            <BannerTitle>Physical Health Goal</BannerTitle>
            <BannerSubtitle>
              Set clear financial goals, track your progress, and take steps
              toward a secure and stress-free future.
            </BannerSubtitle>
            <ViewGoalsButton onClick={handleOpenModal}>
              Add Health Goal
            </ViewGoalsButton>
          </BannerText>
          <BannerImage src={PhysicalBanner} alt="financial Banner" />
        </Banner>
        <Heading>Active Goals</Heading>
        <GoalContainer onClick={handleOpenModal1}>
          <GoalHeader>
            <GoalTitle>Reduce Body Fat</GoalTitle>
...
export default PhysicalGoalModal;
